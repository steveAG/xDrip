...
/**
 * Glucose Graph Tile Service for Wear OS 5
 */
@RequiresApi(api = Build.VERSION_CODES.S)
public class GlucoseGraphTileService extends TileService {
    
    private static final String TAG = "GlucoseGraphTileService";
    ...
    protected ListenableFuture<TileBuilders.Tile> onTileRequest(@NonNull RequestBuilders.TileRequest request) {
        ...
        DeviceParametersBuilders.DeviceParameters deviceParameters = request.getDeviceParameters();
        ...
        if (dg != null) {
            ...
            if (dg.unitized.equals("No Data")) {
                ...
                // Create a column layout with graph
                layoutBuilder.setRoot(
                    new LayoutElementBuilders.Column.Builder()
                        .setModifiers(new ModifiersBuilders.Modifiers.Builder()
                            .setClickable(clickable)
                        ...
                        .addContent(
                            new LayoutElementBuilders.Text.Builder()
                                .setText(dg.mgdl + " " + dg.unitized + " " + dg.delta_arrow)
                                .setFontStyle(new LayoutElementBuilders.FontStyle.Builder()
                                    .setSize(DimensionBuilders.sp(18))
                                    .setColor(ColorBuilders.argb(textColor))
                                    .build())
                                .build())
                        .addContent(createSimpleGraphElement())
                        .addContent(
                            new LayoutElementBuilders.Text.Builder()
    ...
    /**
     * Create a simple graph element
     */
    private LayoutElementBuilders.LayoutElement createSimpleGraphElement() {
        // Create a spacer element to represent a graph
        // In a real implementation, this would be a custom graph drawing
        return new LayoutElementBuilders.Spacer.Builder()
            .setWidth(DimensionBuilders.dp(100))
            .setHeight(DimensionBuilders.dp(80))
            .build();
    }
